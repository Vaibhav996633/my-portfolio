...
interface AboutProps {
  section?: 'full' | 'left-circle' | 'right-content';
}

export const About = ({ section = 'full' }: AboutProps) => {
  const [hoveredTag, setHoveredTag] = useState<string | null>(null);

  if (section === 'left-circle') {
    return (
       <div className="relative h-64 md:h-96 flex items-center justify-center scale-90 md:scale-100">
        {/* Rotating circle background */}
        <motion.div
            className="absolute w-96 h-96 border-2 border-neon-blue rounded-full opacity-30"
            animate={{ rotate: 360 }}
        ...
        <div className="relative z-10">
            <div className="w-80 h-80 rounded-full glass border-2 border-neon-purple p-4 flex items-center justify-center">
            <div className="w-full h-full rounded-full bg-gradient-to-br from-neon-blue/20 to-neon-purple/20 flex items-center justify-center text-9xl">
                📚
            </div>
            </div>
            ...
            {FLOATING_TAGS.map((tag, index) => (
                ...
                style={{
                    left: `${50 + 42 * Math.cos((index / FLOATING_TAGS.length) * Math.PI * 2)}%`,
                    top: `${50 + 42 * Math.sin((index / FLOATING_TAGS.length) * Math.PI * 2)}%`,
  ...
  if (section === 'right-content') {
    return (
        <div className="space-y-6">
            <div className="glass p-8 rounded-xl border border-white/10 backdrop-blur-md">
                <h3 className="text-2xl font-bold text-neon-purple mb-4">Who I Am</h3>
                ...
                <div className="flex flex-col gap-4 border-t border-white/10 pt-4">
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-foreground">Gaming <span className="text-muted-foreground">(strategy, open-world, competitive)</span></span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-2 h-2 bg-neon-green rounded-full" />
                            <span className="text-foreground">Watching movies <span className="text-muted-foreground">(sci-fi & thrillers)</span></span>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="w-2 h-2 bg-neon-green rounded-full" />
                            <span className="text-foreground">Listening to music <span className="text-muted-foreground">(for creativity & relaxation)</span></span>
                        </div>
                    </div>
                </div>
            </div>
