  ...
  useEffect(() => {
    ...
    function startAnim() {
      ...
      objects.forEach(mesh => {
        ...
        (window as any).TweenMax.to(mesh.rotation, 2, {
          x: rx,
          y: ry,
          z: rz,
          delay: delay
        ...
        (window as any).TweenMax.to(mesh.position, 2, {
          z: 80,
          delay: delay + 0.5,
          ease: (window as any).Power1.easeOut
        });

        (window as any).TweenMax.to(mesh.material, 2, {
          opacity: 0,
          delay: delay + 0.5
        });
      });
    ...
    function initObjects() {
      ...
      for (let i = 0; i < nx; i++) {
        for (let j = 0; j < ny; j++) {
          let material = new (window as any).THREE.MeshLambertMaterial({
            color: conf.color,
            transparent: true,
            opacity: 1
          });
    ...
    return () => {
      ...
      window.removeEventListener('load', initScene);
